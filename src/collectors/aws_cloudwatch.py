...
class AWSCloudWatchCollector:
    ...
    def collect_all_metrics(self, days=7):
        """
        Collect metrics for all EC2 instances.
        ...
        for i, instance in enumerate(instances, 1):
            ...
            logger.info(f"[{i}/{len(instances)}] Processing {instance_id} ({instance_state})...")

            # Skip if not running (no metrics available)
            if instance_state != 'running':
                logger.info(f"  ⏭️  Skipping (not running)")
                continue
